...
async function youtubeUpload({ title, videoPath, description, tags, thumbnailPath, visibility, launchOptions }, cookies, event) {
    return new Promise(async (resolve) => {
        ...
        const errorMessage = await page.evaluate(() => document.querySelector('.error-area.style-scope.ytcp-uploads-dialog')?.innerText.trim());
        if (errorMessage) {
            await browser.close();
        ...
        await (0, utils_1.waitUploadComplete)(page, event);
        ...
        await (0, utils_1.$xclickElement)('//*[contains(text(),"No, it\'s")]', page);
        ...
        await (0, utils_1.$xclickElement)(nextBtnXPath, page);
        ...
        if (isChannelMonetized) {
            ...
            try {
                ...
                await page.evaluate(() => (document.querySelector('ytcp-video-monetization-edit-dialog.cancel-button-hidden .ytcp-video-monetization-edit-dialog #radioContainer #onRadio')
                // @ts-expect-error
                )?.click());
                await (0, utils_1.sleep)(1500);
            ...
            try {
                ...
                await page.evaluate(() => (document.querySelector('.ytpp-self-certification-questionnaire .ytpp-self-certification-questionnaire #checkbox-container')
                // @ts-expect-error
                )?.click());
                await (0, utils_1.sleep)(1500);
                ...
                await page.evaluate(() => (document.querySelector('.ytpp-self-certification-questionnaire .ytpp-self-certification-questionnaire #submit-questionnaire-button')
                // @ts-expect-error
                )?.click());
                await page.waitForXPath(nextBtnXPath);
                await page.$x(nextBtnXPath);
        ...
        for (let i = 0; i < 10; i++) {
            try {
                ...
                await (0, utils_1.$xclickElement)(closeDialogXPath, page);
